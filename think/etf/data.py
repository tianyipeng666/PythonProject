...
class DataProvider:
    ...
    def pe_history(self, spec: IndexSpec) -> pd.DataFrame:
        exact_source = self._best_exact_source(spec.code)
        if exact_source:
            self._exact_sources[spec.code] = exact_source
            raw = self.repository.load_valuations(spec.code, exact_source)
    ...
    def pb_history(self, spec: IndexSpec) -> pd.DataFrame | None:
        exact_source = self._exact_sources.get(spec.code) or self._best_exact_source(spec.code)
        if exact_source:
            raw = self.repository.load_valuations(spec.code, exact_source)
    ...
    def _best_exact_source(self, code: str) -> str | None:
        if not self.repository:
            return None
        return self.repository.best_exact_source(code, MIN_EXACT_HISTORY_ROWS)
